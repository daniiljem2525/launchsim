...
(async () => {
  ...
  console.log('— admin credits management');
  r = await req('GET', '/projects');
  const someUserProjectId = r.data?.projects?.[0]?.id;
  cookie = '';
  await req('POST', '/auth/login', { email, password: 'test-pass-123' });
  r = await req('GET', '/auth/me');
  const uid = r.data.user.id;
  const creditsBefore = r.data.user.credits;
  cookie = '';
  await req('POST', '/auth/login', { email: 'admin@launchsim.test', password: 'admin-1234' });
  r = await req('POST', `/admin/users/${uid}/credits`, { amount: 15 });
  check('admin grants 15 credits', r.status === 200 && r.data?.credits === creditsBefore + 15, r.data);
  r = await req('POST', `/admin/users/${uid}/credits`, { amount: -5 });
  check('admin deducts 5 credits', r.status === 200 && r.data?.credits === creditsBefore + 10, r.data);
  r = await req('POST', `/admin/users/${uid}/credits`, { amount: -99999 });
  check('deduct clamps at zero', r.status === 200 && r.data?.credits === 0, r.data);
  r = await req('POST', `/admin/users/${uid}/credits`, { amount: 50 });
  r = await req('POST', `/admin/users/${uid}/credits`, { amount: 0 });
  check('zero amount rejected', r.status === 400, r.status);
  cookie = '';
  await req('POST', '/auth/login', { email, password: 'test-pass-123' });
  r = await req('GET', '/billing');
  check('ledger shows admin adjustments', r.data?.transactions?.some((t) => t.reason?.startsWith('admin:')), r.data?.transactions?.slice(0, 3).map((t) => t.reason));
  console.log('— admin project inspection');
  cookie = '';
  await req('POST', '/auth/login', { email: 'admin@launchsim.test', password: 'admin-1234' });
  r = await req('GET', `/projects/${pid}`);
  check('admin opens any project', r.status === 200 && r.data?.project?.name, r.status);
  check('admin sees project reports list', (r.data?.project?.reports || []).length >= 1);
  cookie = '';
  r = await req('POST', `/admin/users/${uid}/credits`, { amount: 5 });
  check('unauthenticated credit adjust blocked', r.status === 401 || r.status === 403, r.status);
  cookie = '';
