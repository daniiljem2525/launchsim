  ...
  const tryGoogle = async () => {
    try {
      ...
      if (res.status === 501) {
        ...
        setMsg(d.error?.message || 'Google sign-in is not configured here.');
      } else if (res.redirected) {
        window.location.href = res.url;
      }
    } catch { setMsg('Google sign-in is unavailable right now.'); }
  };
  return (
    <div>
...
export function Login() {
  ...
  return (
    <Shell title="Welcome back" sub="Sign in to your LaunchSim workspace.">
      ...
      <div className="center small muted mt-16 mb-16">or</div>
      <GoogleBtn />
...
export function Signup() {
  ...
  return (
    <Shell title="Create your account" sub="Free plan includes 2 credits — enough for your first simulation.">
      <form onSubmit={submit}>
        {err && <div className="banner banner-error">{err}</div>}
        ...
        <div className="field"><label className="label">Email</label>
          <input className="input" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" required /></div>
        <div className="field"><label className="label">Password</label>
          <input className="input" type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Min 8 characters" required minLength={8} /></div>
        <button className="btn btn-primary" style={{ width: '100%' }} disabled={busy}>{busy ? 'Creating…' : 'Create account'}</button>
      </form>
      <div className="center small muted mt-16 mb-16">or</div>
      <GoogleBtn />
      <div className="center small mt-16">Already have an account? <Link to="/login">Sign in</Link></div>
    </Shell>
  );
}
